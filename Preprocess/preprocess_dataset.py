...
parser.add_argument('--input', '-i', default="store/user/tafoyava/samples/nanotron/GluGluHToDarkShowers-ScenarioA_Par-ctau-0p1-mA-0p25-mpi-1_TuneCP5_13p6TeV_powheg-pythia8/RunIII2024Summer24_nanotron_v15-150X_mcRun3_2024_realistic/250812_093711/0000/", type=str, help='Input dataset path')
parser.add_argument('--output', '-o', default="/vols/cms/jtafoyav/parking/QuickChecks/CMSSW_14_0_6_patch1/src/QuickAnalyser/Preprocess/output/MC", type=str, help='Output dataset path')
...
parser.add_argument('--prefix', '-p', default="davs://redirector.t2.ucsd.edu:1095/", type=str, help='Grid prefix')
...
parser.add_argument('--script', default="Preprocess/preprocess_file.py", type=str, help='Script to run on each file')
...
dataset = (args.input).split("/nanotron/")[1].split("_Tune")[0]
output_dataset=args.output+"/"+dataset
...
with open("condor_submission/preprocess_args.txt", "w") as f:
    ...
    for i in range(n_files_to_process):
        ...
        f.write(f"{dataset_input}/{files_filtered[file_index]} {output_dataset}/processed_{files_filtered[file_index]}\n")
...
#export X509_USER_PROXY="/home/hep/jtafoyav/vols/parking/QuickChecks/CMSSW_14_0_6_patch1/src/QuickAnalyser/Preprocess/condor_submission/cms.proxy"
cmsenv
#Run the python script
python3 {script} -i $2 -o $3
#python3 {script} -i $1 -o $2
"""
...
arguments = '/home/hep/jtafoyav/vols/parking/QuickChecks/CMSSW_14_0_6_patch1/src/QuickAnalyser/Preprocess' $(infile) $(outfile)
